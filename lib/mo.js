...
var logType = 'mo';
var addspro = require('./addspro');
var autoMT = require('./autoMT');
...
module.exports = {
    save: function(gateway, req) {
        ...
        db.save('mos', mo).then(result => {
            //log.save('mo (' + gateway + ') saved', logType);
        });
            ...
            db.any('subscribers', filterSubscriber).then(exists => {
                if (exists) {
                    ...
                    db.update('subscribers', filterSubscriber, updateSubscriber).then(result => {
                        log.save('(' + gateway + ') update subscriber [' + mo.service + ']' +
                            '[' + mo.telcoId + ']' +
                            filterSubscriber.msisdn + '/' +
                            filterSubscriber.shortCode + '/' +
                            (filterSubscriber.keyword ? filterSubscriber.keyword : '*'), logType);
                    });
                    ...
                    db.save('subscribers', newSubscriber).then(result => {
                        log.save('(' + gateway + ') new subscriber [' + newSubscriber.service + ']' +
                            '[' + newSubscriber.telcoId + ']' +
                            newSubscriber.msisdn + '/' +
                            newSubscriber.shortCode + '/' +
                            newSubscriber.keyword, logType);
                    });
                }
            });
        } else {
